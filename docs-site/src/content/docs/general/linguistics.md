...
A `Lemma` is the dictionary entry behind an observed form.

For a word like `Seen`, the German lemma might be `See`. For an English form like `ran`, the lemma might be `run`.

The lemma owns properties that belong to the dictionary entry itself:

- `language`: the concrete language, such as `de`, `en`, or `he`
- `canonicalLemma`: the canonical dictionary spelling
- `lemmaKind`: the broad class: `Lexeme`, `Morpheme`, or `Phraseme`
...
The surface always contains a `Lemma`. This keeps a normalized word form connected to the dictionary entry it realizes.
...
`lemmaSubKind` is the public subtype field for all three families. The package does not expose separate public discriminator names like `pos`, `morphemeKind`, or `phrasemeKind`.
